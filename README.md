...
| **Consolidation Adversary** (daily) | `server/consolidation.ts` | `glm-5.1` (when AgentRouter is set) else `BOOP_MODEL` | **Different model family is the point** — adversary's job is to challenge the proposer, and a Claude challenging a Claude tends to echo. GLM-5.1 from a different lineage gives genuine adversarial pressure. **No fallback** on AgentRouter outage — the run is skipped, retried tomorrow. | `BOOP_ADVERSARY_MODEL` |
